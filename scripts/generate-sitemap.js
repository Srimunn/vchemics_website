...
for (const route of staticRoutes) {
  urls.push(`  <url>
    <loc>https://vchemics.com${route.path === "/" ? "/" : route.path}</loc>
...
for (const id of productIds) {
  urls.push(`  <url>
    <loc>https://vchemics.com/products/${id}</loc>
    <lastmod>${today}</lastmod>
...
for (const id of solutionIds) {
  urls.push(`  <url>
    <loc>https://vchemics.com/solutions/${id}</loc>
    <lastmod>${today}</lastmod>
    <changefreq>weekly</changefreq>
    <priority>0.85</priority>
  </url>`);
...
for (const slug of blogSlugs) {
  urls.push(`  <url>
    <loc>https://vchemics.com/blog/${slug}</loc>
    <lastmod>${today}</lastmod>
    <changefreq>weekly</changefreq>
