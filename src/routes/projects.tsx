...
const title = "Projects Portfolio | Infrastructure, Industrial & Commercial Works";
const description =
  "Discover landmark civil and commercial infrastructure projects engineered with V Chemics concrete admixtures, non-shrink grouts, and waterproofing systems across South India.";
      ...
      <section className="bg-background py-16 lg:py-24">
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
            ...
            {categories.map((cat) => (
              <button
                ...
                className={cn(
                  ...
                  activeCategory === cat
                    ? "bg-[#054782] text-white shadow-sm"
          ...
          <div className="mt-12 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {filteredProjects.map((proj, idx) => (
              <Reveal key={proj.id} delay={idx * 60}>
                <div className="group relative flex h-full flex-col justify-between overflow-hidden rounded-3xl border border-border/80 bg-card p-7 shadow-xs transition-all duration-500 hover:-translate-y-1.5 hover:border-[#054782]/40 hover:shadow-2xl">
                  {/* Accent Top Bar */}
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="flex items-center gap-1 font-mono text-xs text-muted-foreground">
                        <Calendar className="h-3.5 w-3.5 text-[#054782]" /> {proj.year}
                      </span>
                    </div>

                    <h3 className="mt-4 font-display text-xl font-bold text-foreground group-hover:text-[#054782] transition-colors leading-snug">
                      {proj.title}
                    </h3>

                    <p className="mt-2 flex items-center gap-1.5 font-mono text-xs font-semibold text-muted-foreground">
                      <MapPin className="h-3.5 w-3.5 text-[#669930] shrink-0" />
                      {proj.location}
                    </p>
                    ...
                    <div className="mt-6 space-y-2">
                      <span className="font-mono text-[0.68rem] font-bold uppercase tracking-wider text-[#054782]">
                  ...
                  <div className="mt-7 pt-4 border-t border-border/60 flex items-center justify-between">
                    <span className="inline-flex items-center gap-1.5 font-mono text-xs font-bold text-[#669930]">
                      <Sparkles className="h-3.5 w-3.5" />
                      {proj.metrics}
                    </span>
                    <span className="inline-flex items-center gap-1 text-xs font-semibold text-[#054782]">
          ...
          <div className="mt-16 rounded-3xl border border-border/80 bg-gradient-to-r from-[#0b274c] to-[#071933] p-8 sm:p-12 text-white shadow-xl relative overflow-hidden">
            <div className="absolute -right-20 -bottom-20 h-64 w-64 rounded-full bg-[#669930]/20 blur-3xl pointer-events-none" />
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
              <div className="space-y-2 text-center md:text-left">
                ...
                <p className="text-sm text-[#b0c7df] max-w-xl">
                  We supply custom trial batches, concrete mix design calibrations, and on-site application supervision for large-scale infrastructure and industrial projects.
                </p>
              </div>

              <Link
                to="/contact"
                className="shrink-0 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#054782] to-[#669930] px-8 py-4 font-display text-sm font-bold uppercase tracking-wider text-white shadow-md transition-all hover:scale-105"
              >
